...
raw_data_path = sys.argv[4]

data_types = []
if (len(sys.argv[5:]) > 0):
	for i in range(len(sys.argv[5:])):
		if (sys.argv[5+i] not in AVAILABLE_DATA_TYPES):
			print("Bad data types")
			sys.exit(1)
		data_types.append(sys.argv[5+i])
else:
...
for data_type in data_types :
	...
	for time in range(0, 24):
		...
		plt.savefig("meteo_outputs/"+str(data_type)+"/image_"+str(time)+".png")
		plt.close()